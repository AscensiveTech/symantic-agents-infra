...
resource "aws_s3_bucket" "knowledge_assets" {
  bucket = "${local.name_prefix}-knowledge-assets"

  tags = {
    Name = "${local.name_prefix}-knowledge-assets"
  }
}

resource "aws_s3_bucket_public_access_block" "knowledge_assets" {
  bucket                  = aws_s3_bucket.knowledge_assets.id
  block_public_acls       = true
  block_public_policy     = true
  ignore_public_acls      = true
  restrict_public_buckets = true
...
resource "aws_s3_bucket_server_side_encryption_configuration" "knowledge_assets" {
  bucket = aws_s3_bucket.knowledge_assets.id
...
resource "aws_s3_bucket_cors_configuration" "knowledge_assets" {
  bucket = aws_s3_bucket.knowledge_assets.id
